...
const CitizenForm: React.FC<CitizenFormProps> = ({ onSubmission, currentUser, myComplaints, onUpdateUser }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsVerifying(true);
    try {
      // Verification happens silently as requested
      ...
      // Clear form and switch page as requested
      setImage(null);
      setDescription('');
  ...
  return (
    <div className="space-y-6">
      <div className="flex bg-white p-2 rounded-[2.5rem] shadow-sm border border-slate-100 overflow-x-auto no-scrollbar">
        {['report', 'profile', 'problems', 'points'].map((tab) => (
          <button
            ...
            className={`flex-1 py-4 px-6 text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl transition-all whitespace-nowrap ${activeTab === tab ? 'bg-blue-600 text-white shadow-xl shadow-blue-200' : 'text-slate-400 hover:bg-slate-50'}`}
          >
            {tab}
          </button>
        ))}
          ...
          <div className="p-10">
            <div className="mb-10 p-8 bg-blue-50/50 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between border border-blue-100 gap-6">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest">Deployment Zone</p>
                  <p className="text-xl font-black text-blue-900">{currentUser.city}</p>
                </div>
              </div>
              <div className="text-center md:text-right">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Potential Impact Reward</p>
                <p className="text-2xl font-black text-emerald-600">+{CATEGORY_POINTS[category]} PTS</p>
              </div>
            </div>
                ...
                <div className="space-y-10">
                  <div>
                    <div className="flex justify-between items-center mb-6">
                      <label className="block text-xs font-black text-slate-700 uppercase tracking-[0.2em]">Identify Problem Type</label>
                      {isAnalyzing && <span className="text-[9px] font-black text-blue-600 animate-pulse uppercase"><i className="fas fa-sparkles mr-2"></i> AI Auto-Detecting...</span>}
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      {Object.values(ComplaintCategory).map((cat) => (
                        <button 
                          ...
                          className={`p-5 text-[10px] font-black uppercase tracking-widest rounded-2xl border-2 transition-all ${category === cat ? 'bg-blue-600 border-blue-600 text-white shadow-xl shadow-blue-100' : 'border-slate-100 text-slate-400 hover:bg-slate-50 hover:border-slate-300'}`}
                        >
                          {cat.replace('_', ' ')}
                        </button>
                      ))}
                  ...
                  <div>
                     <div className="flex justify-between items-center mb-4">
                       <label className="block text-xs font-black text-slate-700 uppercase tracking-[0.2em]">Incident Summary</label>
                       {isAnalyzing && <span className="text-[9px] font-black text-blue-600 animate-pulse uppercase"><i className="fas fa-pen-nib mr-2"></i> AI Drafting...</span>}
                     </div>
                     <textarea className="w-full h-52 p-8 rounded-[3rem] border-2 border-slate-100 bg-slate-50/30 outline-none focus:ring-4 focus:ring-blue-50 focus:border-blue-500 font-bold text-slate-700 transition-all resize-none shadow-inner" placeholder="Provide context of the infrastructure issue..." value={description} onChange={(e) => setDescription(e.target.value)} required />
                  </div>
                </div>

                <div className="space-y-10">
                  <div>
                    <label className="block text-xs font-black text-slate-700 mb-6 uppercase tracking-[0.2em]">Visual Evidence (GPS Photo)</label>
                    <div className={`relative h-80 border-4 border-dashed rounded-[3.5rem] transition-all flex items-center justify-center bg-slate-50/50 ${image ? 'border-emerald-500 bg-emerald-50/10' : 'border-slate-100 hover:border-blue-400 hover:bg-white shadow-inner'}`}>
                      {image ? (
                        ...
                        <div className="text-center p-8">
                          <div className="w-24 h-24 bg-white rounded-[2.5rem] shadow-xl flex items-center justify-center text-blue-500 text-3xl mb-6 mx-auto border border-slate-100">
                            <i className="fas fa-camera"></i>
                          </div>
                          <div className="flex gap-4 justify-center">
                            <button type="button" onClick={() => fileInputRef.current?.click()} className="px-6 py-3 bg-white border-2 border-slate-100 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 active:scale-95 transition-all shadow-sm">From Gallery</button>
                            <button type="button" onClick={() => cameraInputRef.current?.click()} className="px-6 py-3 bg-white border-2 border-slate-100 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 active:scale-95 transition-all shadow-sm">Open Camera</button>
                          </div>
                        </div>
                      )}
                  ...
                  <div className="space-y-6">
                    ...
                    {location && (
                      <div className="p-8 bg-slate-900 rounded-[2.5rem] border border-slate-800 space-y-6 animate-slideDown shadow-2xl relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-4 relative z-10">
                           ...
                           <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Spatial Deployment Data</p>
                        </div>
                        <div className="grid grid-cols-2 gap-6 relative z-10">
                          <div className="bg-white/5 p-5 rounded-2xl border border-white/5">
                            <span className="block text-[8px] font-black text-slate-500 uppercase mb-2 tracking-widest">NORTH LATITUDE</span>
                            <span className="text-sm font-mono font-bold text-white">{location.lat.toFixed(8)}</span>
                          </div>
                          <div className="bg-white/5 p-5 rounded-2xl border border-white/5">
                            <span className="block text-[8px] font-black text-slate-500 uppercase mb-2 tracking-widest">EAST LONGITUDE</span>
      ...
      {activeTab === 'profile' && (
        <div className="bg-white rounded-[3.5rem] p-16 border border-slate-100 shadow-2xl animate-fadeIn">
          <div className="flex flex-col items-center text-center mb-16">
            ...
            <div className="mt-4 px-8 py-2.5 bg-slate-100 rounded-full text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 border border-slate-200">Public Servant ID: {currentUser.id.slice(0, 8).toUpperCase()}</div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-4xl mx-auto">
            <div className="space-y-8">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.3em] border-l-4 border-blue-600 pl-6">Profile Metadata</h3>
              <div className="space-y-4">
                <div className="p-7 bg-slate-50/50 rounded-[2.5rem] flex justify-between items-center border border-slate-100 group hover:bg-white transition-all">
                ...
                <div className="p-7 bg-slate-50/50 rounded-[2.5rem] flex justify-between items-center border border-slate-100 group hover:bg-white transition-all">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Gmail Access</span>
                  <span className="text-sm font-black text-slate-800 break-all ml-10">{currentUser.email}</span>
                </div>
              </div>
            </div>
            <div className="space-y-8">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.3em] border-l-4 border-emerald-600 pl-6">Localized Jurisdiction</h3>
              <div className="space-y-4">
                <div className="p-7 bg-slate-50/50 rounded-[2.5rem] flex justify-between items-center border border-slate-100 group hover:bg-white transition-all">
                ...
                <div className="p-7 bg-slate-50/50 rounded-[2.5rem] flex justify-between items-center border border-slate-100 group hover:bg-white transition-all">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Municipal Hub</span>
      ...
      {activeTab === 'problems' && (
        <div className="space-y-10 animate-fadeIn">
          ...
          {myComplaints.length === 0 ? (
            <div className="bg-white p-32 text-center rounded-[4rem] border-4 border-dashed border-slate-100 flex flex-col items-center">
              <div className="w-24 h-24 bg-slate-50 rounded-[3rem] flex items-center justify-center text-slate-200 text-4xl mb-8">
                 <i className="fas fa-layer-group"></i>
              </div>
              <p className="text-slate-400 font-black uppercase tracking-[0.3em] text-sm">No municipal reports filed yet.</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 px-4">
              {myComplaints.map(c => (
                <div key={c.id} className="bg-white rounded-[3.5rem] border border-slate-100 p-8 flex flex-col sm:flex-row gap-8 hover:shadow-[0_40px_80px_-15px_rgba(0,0,0,0.1)] transition-all group relative overflow-hidden">
                  <div className="w-full sm:w-44 h-44 rounded-[3rem] overflow-hidden shadow-inner flex-shrink-0 relative">
                    ...
                    <div className="absolute top-4 left-4 px-4 py-1.5 bg-white/95 backdrop-blur-md rounded-full text-[9px] font-black uppercase tracking-widest shadow-lg border border-slate-50">{c.category.replace('_', ' ')}</div>
                  </div>
                  <div className="flex-1 flex flex-col justify-center py-2">
                    <div className="flex justify-between items-center mb-4">
                      <span className={`text-[10px] font-black uppercase tracking-widest px-5 py-1.5 rounded-full shadow-sm border ${
                        c.status === ComplaintStatus.RESOLVED ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                        c.status === ComplaintStatus.IN_PROGRESS ? 'bg-blue-50 text-blue-700 border-blue-100' :
                        c.status === ComplaintStatus.REJECTED ? 'bg-red-50 text-red-700 border-red-100' :
                        'bg-amber-50 text-amber-700 border-amber-100'
                      }`}>{c.status}</span>
                      <span className="text-[10px] font-black text-slate-300">TOKEN: #{c.id.slice(0, 5).toUpperCase()}</span>
                    </div>
                    <p className="text-base font-bold text-slate-700 line-clamp-2 leading-relaxed mb-6">{c.description}</p>
                    <div className="flex items-center gap-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                       ...
                       {c.assignedTeamId && (
                         <div className="flex items-center gap-2">
                           <i className="fas fa-truck-fast text-blue-400"></i>
                           <span>Deployment Unit Active</span>
                         </div>
                       )}
                    </div>
                  </div>
      ...
      {activeTab === 'points' && (
        <div className="bg-white rounded-[5rem] p-24 border border-slate-100 shadow-2xl animate-fadeIn text-center relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="inline-flex items-center justify-center w-48 h-48 bg-white shadow-2xl rounded-[4rem] mb-12 border border-slate-50 relative group cursor-pointer active:scale-95 transition-all">
            ...
            <h2 className="text-8xl font-black text-slate-900 mb-4 tracking-tighter">{resolvedPoints} <span className="text-3xl text-slate-300 tracking-widest ml-2">CREDITS</span></h2>
            <p className="text-slate-400 font-black uppercase tracking-[0.5em] text-[12px] mb-16">Verified Municipal Impact Balance</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 text-left max-w-4xl mx-auto">
              <div className="p-12 bg-slate-900 rounded-[3.5rem] text-white shadow-2xl relative overflow-hidden">
                ...
                <h4 className="text-[11px] font-black text-slate-500 uppercase mb-8 tracking-[0.3em] flex items-center gap-4">
                ...
                <div className="space-y-6">
                  {Object.entries(CATEGORY_POINTS).map(([cat, pts]) => (
                    <div key={cat} className="flex justify-between items-center text-[12px] font-black uppercase tracking-widest group cursor-default">
                      <span className="text-slate-400 group-hover:text-white transition-colors">{cat.replace('_', ' ')}</span>
              ...
              <div className="p-12 bg-blue-600 rounded-[3.5rem] text-white flex flex-col justify-center text-center shadow-2xl shadow-blue-200 group hover:-translate-y-2 transition-all">
                <p className="text-[11px] font-black uppercase tracking-[0.4em] mb-6 text-blue-200">Civic Tier Rank</p>
                ...
                <p className="text-[11px] font-black mt-8 uppercase text-blue-100 tracking-[0.2em]">{100 - (resolvedPoints % 100)} PTS until next echelon</p>
              </div>
            </div>
          </div>
        </div>
